...
JSON and Markdown QA reports include complete captured event payloads and request metadata. Reports are generated locally only after the user acknowledges the raw client-data notice and explicitly exports or copies a report. They are never uploaded by the extension. Because these files can contain client data, identifiers, or URLs, handle them according to the client's approved QA evidence and retention process.
