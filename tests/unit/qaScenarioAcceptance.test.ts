...
describe('multi-step QA scenario acceptance', () => {
  it('completes pass, warn, and fail steps across navigation and exports all evidence', () => {
    ...
    const report = createExportReport(postNavigationSession, '0.4.0', run);
    const jsonReport = JSON.parse(exportReportJson(postNavigationSession, '0.4.0', run));
    const markdownReport = exportReportMarkdown(postNavigationSession, '0.4.0', run);
